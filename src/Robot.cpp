...
void Robot::RobotInit() {
	...
	centerGoalAuto.reset(new CenterGoal(63, 160, 5, 10));

//	CameraServer::GetInstance()->StartAutomaticCapture("cam0");
	networkTable = NetworkTable::GetTable("GRIP");

//	RobotMap::shootingKickupSpike->Set(Relay::kOn);
	printf("Robot inited\n");
...
//	}
	...
	printf("Auto init\n");
	try {
	if (centerGoalAuto.get() != nullptr) {
		centerGoalAuto->Start();
	}
	}
	catch (std::exception *e) {
		printf("Auto init exception: %s", e->what());
	}
}
...
void Robot::AutonomousPeriodic() {
	...
	Scheduler::GetInstance()->Run();
		...
		printf("Auto periodic exception: %s", e->what());
	}
}
